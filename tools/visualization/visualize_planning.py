...
def draw_plan(sample_token, nusc, plan_trajs_dict, output_path):
    ...
    plan_traj_str = plan_trajs_dict['predict']
    gt_traj_str = plan_trajs_dict['label']

    # Parse GT trajectory
    gt_traj = None
    try:
        numbers = re.findall(r'\((-?\d+\.\d+),(-?\d+\.\d+)\)', gt_traj_str)
        if numbers:
            gt_traj = np.concatenate([[(float(x), float(y)) for x, y in numbers], np.ones((6,1))], axis=-1)
    except Exception:
        pass
    ...
    plan_traj = None
    try:
        numbers = re.findall(r'\((-?\d+\.\d+),(-?\d+\.\d+)\)', plan_traj_str)
        if numbers:
            plan_traj = np.concatenate([[(float(x), float(y)) for x, y in numbers], np.ones((6,1))], axis=-1)
    except Exception:
        pass

    # If neither trajectory is available, skip
    if gt_traj is None and plan_traj is None:
        return
